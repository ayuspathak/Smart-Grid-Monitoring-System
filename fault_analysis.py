...
def calculate(fault_type, feeder, resistance=0.08):
    ...
    multiple=max(current/0.40,1.0)
    clearing=max(35.0,min(180.0,115.0/(multiple**1.45)))
    ...
    return {"fault_type":fault_type,"feeder":feeder,"fault_current_ka":round(current,3),"clearing_ms":round(clearing,1),"severity":severity,"voltage_profile":profile}
